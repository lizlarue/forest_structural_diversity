...
coverS <- loadByProduct (dpID = "DP1.10058.001", site = 'SCBI')
...
SCBI_table <- cbind(SCBI_structural_diversity, all_SR, exotic_SR)
...
write.table(combo11, file = "prelim_results.csv", sep = ",", row.names = FALSE)

library(ggplot2)
ggplot(combo11, aes(x = mean.max.canopy.ht.aop, y = exotic_SR))+
  geom_point()

ggplot(combo11, aes(x = max.canopy.ht.aop, y = exotic_SR))+
  geom_point()

ggplot(combo11, aes(x = rumple.aop, y = exotic_SR))+
  geom_point()
